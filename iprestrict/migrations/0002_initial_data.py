...
def create_data(apps, schema_editor):
	...
	IPRange.objects.create(
		ip_group=all_group, first_ip="0.0.0.0", last_ip="255.255.255.255"
	)
	IPRange.objects.create(
		ip_group=all_group,
	...
	IPRange.objects.create(ip_group=localhost_group, first_ip="127.0.0.1", last_ip=None)
	IPRange.objects.create(ip_group=localhost_group, first_ip="::1", last_ip=None)

	# Default rules: Allow all for localhost
	Rule.objects.create(
		ip_group=localhost_group, action="A", url_pattern="ALL", rank=65535
	)
...
class Migration(migrations.Migration):
	...
	operations = [
		migrations.RunPython(create_data, reverse_code=reverse_create_data),
	]
